...
<!-- <?php else: ?>
    <div class="row" style="margin-top: 170px;margin-bottom: 10px;">
<?php endif ?> -->
            <div class="row">
                ...
                <div class="col-6">
                    ...
                    <?php if ($i->link_type == 'makanan'): ?>
                        <?php if ($cekSignature): ?>
                            <?php  if ($cekSignature->sort == 0): ?>
                                href="<?=  base_url() ?><?= $i->link ?><?= str_replace(" ","%20", 'Signature') ?>#<?= str_replace(" ","_", 'Signature') ?>"
                            <?php else: ?>  
                                href="<?=  base_url() ?><?= $i->link ?><?= str_replace(" ","%20", $sca) ?>#<?= str_replace(" ","_", $sca) ?>"
                            <?php endif ?>
                        <?php else: ?>
                            href="<?=  base_url() ?><?= $i->link ?><?= str_replace(" ","%20", $sca) ?>#<?= str_replace(" ","_", $sca) ?>"
                        <?php endif ?>
                    <?php elseif ($i->link_type == 'minuman'): ?>
                        href="<?=  base_url() ?><?= $i->link ?><?= str_replace(" ","%20", $scm) ?>#<?= str_replace(" ","_", $scm) ?>"
                    <?php elseif ($i->link_type == 'paket' || $i->link_type == 'status'): ?>
                    ...
                    <?php endif ?>
                     ...
                     class="btn" style="display: flex; flex-direction: column; align-items: center;">
                        ...
                        <span style="margin-top: 5px;margin-bottom: 10px;font-size: 15px;font-weight: bold;color: <?= $cn->color ?>"><?= $i->title ?></span>
                    </a>
                </div>
                <?php endforeach ?>
            </div>
