...
<div class="head" style="margin-bottom: 200px;background: linear-gradient(to right, <?= $cn->lightcolor ?>, <?= $cn->color ?>, <?= $cn->darkcolor ?>);color: white;">
    <header>
        <div style="display: flex; align-items: center;">
            <a href="<?= $linkback ?>" style="text-decoration: none; color: white;">
...
<form action="<?= base_url() ?>index.php/review/save/<?= $nomeja;?>/<?= $sub ?>" method="post">
  ...
  <footer>
    <div class="containerfooter" style="padding: 10px;">
        <button type="submit" class="btn add-btn" style="padding: 15px;font-size: 17px;background-color: <?= $cn->color ?>;color: white;"><strong>Send</strong></button>
    </div>
  </footer>
