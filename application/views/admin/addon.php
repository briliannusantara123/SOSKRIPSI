      	...
      	<form action="<?= base_url('index.php/admin/create_addon') ?>" method="POST">
	        <div class="mb-3">
			  ...
			  <select class="form-control" name="no" required="">
			  	<?php foreach ($item as $i): ?>
				    <?php 
				        // Check if the item number exists in the existing_items array
				        $is_added = in_array($i->no, $cekADDON) ? ' ( Already Added )' : ''; 
				    ?>
			...
			<div class="mb-3">
			  ...
			  <select class="form-control" name="addon" required="">
			  	<?php foreach ($item as $i): ?>
				    <?php 
				        // Check if the item number exists in the existing_items array
				        $is_added = in_array($i->no, $cekADDONDESC) ? ' ( Already Added )' : ''; 
				    ?>
				    <option value="<?= $i->no ?>"><?= $i->description . $is_added ?></option>
				<?php endforeach ?>
			  </select>
