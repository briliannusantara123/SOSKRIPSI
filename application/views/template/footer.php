...
<input type="hidden" id="submakanan" value="<?= str_replace(" ","%20", $submakanan) ?>#<?= str_replace(" ","_", $submakanan) ?>">
<input type="hidden" id="subminuman" value="<?= str_replace(" ","%20", $subminuman) ?>#<?= str_replace(" ","_", $subminuman) ?>">
  <script>
    ...
    function scrollToBottom() {
        ...
        <?php if( $this->session->flashdata('success') ) {?>
            ...
            showConfirmButton: false, // hilangkan tombol OK
                allowOutsideClick: true,  // klik di luar untuk close
                allowEscapeKey: true,     // tekan ESC untuk close
                backdrop: true            // aktifkan klik background
            })
         <?php } ?>
        ...
        <?php } ?>
         <?php if ($this->session->flashdata('successkasir')) { ?>
            var isi = <?= json_encode($this->session->flashdata('successkasir')) ?>;
            Swal.fire({
                title: '',
                text: isi,
                icon: 'warning',
                showConfirmButton: false,   // ⬅️ hilangkan tombol OK
                allowOutsideClick: true,    // ⬅️ klik luar untuk close
                allowEscapeKey: true        // ⬅️ tombol ESC juga bisa close
            });
        <?php } ?>
